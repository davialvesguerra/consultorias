...
#variáveis para fazer tabela:

#Tempo de docência no SEEDF     : 25
#Coordenação regional de ensino : 26
#Modalidade de Ensino atual     : 27
#Vínculo institucional no SEEDF : 28
...
```{r}
barras(data1, "Cor",save=T,dir_ = "images")
```


Quanto à idade dos discentes, a predominância é na faixa dos 30 aos 49 anos, representando mais de 71\%. A faixa dos 40 a 49 anos tem a maior concentração, com 40\%. A faixa dos 55 anos ou mais apresenta a menor representação, com 6,67\%.

\newpage
```{r}
barras(data1, "Idade",save=T,dir_ = "images",is_ordem_vars=T,ordem_vars =  c("De 25 a 29 anos","De 30 a 39 anos","De 40 a 49 anos","De 50 a 54 anos","55 anos ou mais"))
```
...
```{r}
barras(data1, "Sexo",save=T,dir_ = "images")
```
...
```{r}
barras(data1, "escolaridade",save=T,dir_ = "images", xlab = T, nome_coluna = "Escolaridade", is_ordem_vars = TRUE,ordem_vars =  c("Graduação","Especialização","Mestrado"))
```
...
```{r}
barras(data1, "Realizou alguns dos cursos das oficinas pedagógicas da SEEDF", save=T,dir_ = "images")
```
...
# Variável: Atividades que contribuiram para o trabalho docente
